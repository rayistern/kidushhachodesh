  ...
  sections: [
    {
      ...
      body: [
        ...
        'The sun again. In chapter 14 the sun\'s position stood in for the time of year; here it does something quite different, and this time the reason is physical rather than a matter of timing.',
      ],
    },

    {
      ...
      body: [
        ...
        'Why double? The commentary in the **Chitrik edition** gives a reason, and it is much better than it looks: **the small circle\'s distance from the sun is always exactly half its distance from the far point** (נקודת הגובה) of the big circle the small one rides round — the same kind of far point the sun had in chapter 12. (The small circle is chapter 14\'s: the one the moon rides on the end of the arm. And "distance" here means what it always means in this book — **degrees apart against the sky**, how far round the circle one thing is from another. Not miles. The far point is *named* for being physically furthest from us, but its distance from anything is still measured as an angle.)',
        ...
        'It also explains why the short table of nudges coming two sections below starts at nothing. When sun and moon are together the gap is zero, so the small circle is sitting **on** the far point — and there is nothing to correct, exactly as the sun\'s correction vanishes at its own govah.',
        'Why does the lock hold? Two facts. At new moon the far point sits exactly where the sun is, so **both distances start at zero together**. And from there the moon pulls away from each at a fixed speed — twice as fast from the far point as from the sun, because the far point travels **backwards**, about 11 degrees a day — once round in roughly 32 days — at just the speed that makes the ratio exact. Two things that start at zero together and grow at two-to-one stay at two-to-one forever.',
        'The backwards march is not a coincidence; it is built in. That speed is chosen precisely so the lock holds — it is the gearing by which the model encodes what the sky shows. The Rambam never mentions any of it. He does not have to, and that is the elegance of it.',
        'And what is all this machinery *for*? The nudge it feeds — the next section — corrects a real disturbance: the sun genuinely drags the moon off its simple path, and by how much depends on how the moon, the sun and the far point stand relative to one another, which is exactly what the doubled gap measures. Modern astronomy calls that wobble the *evection*; after the in-and-out of chapter 14\'s small circle, it is the biggest of the moon\'s wobbles. The Rambam has no theory of gravity to explain it, but he has the pattern, and the pattern is all the table needs.',
      ],
...

    {
      ...
      body: [
        ...
        'Which is also why his table in the next halacha stops at 63. He never needs more, so his table never lists more.',
      ],
    },
