  ...
  sections: [
...

    {
      ...
      body: [
        ...
        'The fraction is real; it is simply not in the block. A true lunar month is **29 days and about 12¾ hours**, so his row falls half a day short of one. The fraction lives in the *rate*, which is why the rate carries a third of a second; the block is a whole number of days by construction, because whole days are all the method ever handles.',
        '**Second, and this is the useful part: the blocks are never chained.** A previous month\'s value is never needed, because there is no month-to-month running total anywhere in the method. Every calculation goes back to the same fixed evening in 1178 and asks one question: *how many days from then to now?* Nothing carries forward, because there is no previous month in the arithmetic.',
        'That also means these two rows are **optional**. Drop them and you can still reach every answer in the book with the four round blocks and single days — 29 days is just two tens and nine singles. What they are for is a **check**: his published figure and the one you assemble from the blocks are two routes to the same number, so a disagreement means a mistake.',
        'The labels do make sense once you see where they come from — they are **calendar** lengths, not astronomical ones. Twenty-nine days is a short Hebrew month; 354 days is a common Hebrew year, one of the six lengths from chapter 11. He is pre-computing the spans his reader would actually be handed.',
        'Checked against the sky, one label is honest and one is not. 354 days really is a lunar year — twelve true lunar months come to 354.37 days, within nine hours. But it is eleven days short of a **solar** year, which is the gap the leap month exists to close. And twelve of his 29-day blocks give 348 days, over six days shy of a lunar year. **The blocks do not compose into each other.**',
      ],
    },
