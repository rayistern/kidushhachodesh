  ...
  sections: [
...

    {
      ...
      'That also explains something chapter 17 never does. His thresholds come in two sets — one for the 10th sign through the 3rd, one for the 4th through the 9th — and the tables simply have two columns. Those two halves are exactly the climbing half and the falling half, split at the turning points. He never explains the split; this is what it is.',
        'Three plainer reasons on top of that. In the era of the witnesses, a sign could be **checked against the sky** — nobody can eyeball "104 degrees", but a judge could see which star-picture the moon stood in, because in courtroom times the pictures still sat inside their signs (the slow drift described two paragraphs down had only reached a few degrees). That reason has since expired — today the pictures sit a whole sign away — and it is the only one of the three that has. It **catches mistakes**: slip thirty degrees in your addition and you land in the wrong sign, which is glaring, where a wrong number looks like any other number. And it was **the language everyone already spoke** — witnesses and judges talked in constellations, so the arithmetic had to come back out in words the court used.',
        'As for the names themselves — Taleh, Shor, Teomim, and the nine that follow — they are handles and not much more. What you actually need is **the order**, and that each one is thirty degrees wide. That is why the rest of this book usually says "the 2nd sign" and puts the name in brackets: the number is the useful part, and it saves you memorising twelve unfamiliar words before you can read a sentence. The Rambam all but says so: he notes that the stars have shifted since these were named, so the pictures no longer really fit. Today that shift has grown to roughly **a full sign** — the sky\'s whole star-frame slides about 1 degree every 72 years, and some two thousand years have passed since the naming era — so when this book says the sun stands at the start of the 1st sign (Taleh), the stars behind it are actually the 12th sign\'s picture, the fish of Dagim. As pointers to actual star groups, the names are useless now. Nothing in the method minds: chapter 9 anchors the 1st sign to the spring season-point, not to any star, which is exactly why the whole calculation still works after eight centuries. Do not spend effort on the imagery. Learn the order.',
