  ...
  sections: [
...

    {
      ...
      body: [
        ...
        'It is worth being careful about what those two laps *mean*, because the natural way to describe them goes beyond anything the Rambam says. His own words (14:1) are only that the moon "revolves in a small orbit that does not encompass the earth", and that the small orbit "itself rotates in a larger orbit that encompasses the earth". That is the structure, and nothing more.',
        'From that structure one thing does follow on its own. If the moon rides a small circle that does not go round the earth, then there is a point of that circle nearest us and a point furthest away — so a lap of the small circle is also a lap of **near and far**. That is a fair deduction from his model. It is not a quotation: he does not discuss the moon\'s distance changing here at all, and calling the second period "closest approach to closest approach" is a modern gloss on his figure rather than his description of it.',
        'And the first lap deserves the same care. It is tempting to say "back to the same star", but that is the one reading his numbers do not support. The circle he counts round is anchored to the seasons, not the stars — the same tropical circle chapter 13 used. As it happens the two kinds of month differ by only about seven seconds, so his figure cannot tell them apart anyway; it sits between them, a second and a half from one and five and a half from the other. Precise enough to be remarkable, not precise enough to settle which.',
