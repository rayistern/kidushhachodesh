 ...
 *
 * The check is honest about where it does not fit: the shipped table is
 * lopsided (+15' at most on one side, −30' on the other) while sunset
 * drift is very nearly symmetric. That mismatch is left visible rather
 * than smoothed, and it is one of the reasons a competing reading of
 * the table exists — see SeasonBands.
 */
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-[11px] leading-relaxed text-[var(--color-text-secondary)]">
        Where it does <em>not</em> fit: the steps are lopsided. He goes down to −30′ in winter but
        only up to +15′ in summer, while sunset drift is nearly symmetric. Hold on to that — it
        is exactly the row the printed texts argue about, and the next figure takes it up.
      </p>
  ...
  return (
    <figure>
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full" role="img"
        ...
        {[30, -30].map((v) => (
          <g key={v}>
            <line x1={padL} y1={y(v)} x2={w - padR} y2={y(v)} stroke="var(--color-border)" strokeWidth="1" strokeDasharray="2 4" />
